<template>
  <div style="background:#fff ;padding:25px;position: relative">
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="handleEntityCategory('add')">添加</a-button>
      <a-button type="primary" icon="edit" @click="handleEntityCategory('edit')">编辑</a-button>
      <a-button type="primary" icon="delete" @click="handleEntityCategory('delete')">删除</a-button>
      <a-dropdown>
        <a-menu slot="overlay">
          <a-menu-item key="1" @click="handleEntityCategory('enabled')">
            <a-icon type="unlock" />启用
          </a-menu-item>
          <a-menu-item key="2" @click="handleEntityCategory('disabled')">
            <a-icon type="lock" />禁用
          </a-menu-item>
        </a-menu>
...
export default {
  ...
  methods: {
    ...
    handleOk() {
      ...
      if (_type == 'delete') {
        this.deleteEntityCategory()
      } else if (_type == 'enabled' || _type == 'disabled') {
        this.setEntityCategoryState()
      } else {
        const flag = this.checkData()
        flag && this.addOrEditEntityCategory()
      }
    },

    // 操作店铺
    handleEntityCategory(e, o) {
      this.editType = e
        ...
        .then(res => {
          ...
          if (res.code == 0) {
            if (res.result.length > 0) {
              this.setEntityCategorySelect(res.result)
            } else {
        ...
        .then(res => {
          ...
          if (res.code == 0) {
            ...
            if (res.page.list.length > 0) {
              this.setEntityCategoryShow(res.page.list)
            } else {
              this.loadDatas = []
            }
          } else {
