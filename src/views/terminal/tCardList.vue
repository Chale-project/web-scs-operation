  ...
  <div style="background:#fff ;padding:25px;position: relative">
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="48">
          <a-col :md="8" :sm="24">
            <a-form-item label="台卡编号">
              <a-input v-model="queryParam.polymerizationCode" placeholder="请输入台卡编号" />
            </a-form-item>
          </a-col>
          <a-col :md="8" :sm="24">
            <a-form-item label="台卡状态">
              <a-select placeholder="请选择" v-model="queryParam.state">
                <a-select-option value="">全部</a-select-option>
                <a-select-option value="notBand">未使用</a-select-option>
                <a-select-option value="band">已使用</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="handleTCard('add')">批量生成台卡</a-button>
    </div>
    ...
    <a-table size="middle" :columns="columns" :dataSource="loadDatas" :loading="loading" :pagination="pagination" :rowSelection="{type:'checkbox', selectedRowKeys: selectedRowKeys, onChange: updateSelect }">
      ...
      <template slot="state" slot-scope="record">
        <a-tag v-if="record=='notBand'" color="#87d068">未使用</a-tag>
        <a-tag v-else-if="record=='band'" color="#ff0000">已使用</a-tag>
      </template>
    ...
    <a-modal :width="editType=='add'?'400px':'400px'" :title="modalTitle" :visible="visible" @ok="handleOk" :confirmLoading="confirmLoading" @cancel="handleCancel">
      <div class="modal-container">
        <a-form>
          <a-row :gutter="24">
            <a-col :md="24" :sm="24">
              <a-form-item label="生成台卡数量" :required="true">
                <a-input type="number" v-model="forms.number" addonAfter="个" placeholder="请输入一次性生成台卡数量" />
              </a-form-item>
            </a-col>
          </a-row>
        </a-form>
...
import { addTCard, getTCardList } from '@/api/common'

const columns = [
  {
    title: '台卡编号',
    width: '10%',
    dataIndex: 'polymerizationCode'
  },

  {
    title: '台卡',
    width: '10%',
    dataIndex: 'polymerizationUrl',
    scopedSlots: { customRender: 'polymerizationUrl' }
  },
